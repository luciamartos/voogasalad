...
/**
 * This utility provides convenience methods for dynamically calling methods and
 * allocating classes.
 * 
...
public class LevelWindowFactory {
	...
	// could use a resource file, but that seems like an extra burden to use
	// this utility
	 ...
	 */
	public static AbstractLevelEditorWindow createInstance(String name, Object... args) throws ReflectionException {
		try {
			...
			for (Constructor<?> current : c.getDeclaredConstructors()) {
				...
				if (typesMatch(current, formals, args)) {
					return (AbstractLevelEditorWindow) current.newInstance(convertArgs(current, formals, args));
				}
			}
