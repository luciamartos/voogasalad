...
public class LevelWindowPaneFactory {
	...
	private Sprite findSprite(String nodeId) {
		...
		return this.authorController.getModel().getGame().getPresets().stream().filter(s -> s.getId().equals(nodeId))
				.findAny().orElse(null);
		// this.authorController.getModel().getGame().getPresets().stream().filter(s
		// -> {
		// nodeId.equals(s.getId()).findFirst().orElse(null);
		// });

		// for (Sprite s :
		// this.authorController.getModel().getGame().getPresets()) {
